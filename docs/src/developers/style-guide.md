...
!!! info
    The style guide is always a work in progress, and not all SimplePhaseDiagrams code
    follows the rules. When modifying SimplePhaseDiagrams, please fix the style violations
...
SimplePhaseDiagrams uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/SimplePhaseDiagrams.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the SimplePhaseDiagrams directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to SimplePhaseDiagrams have
