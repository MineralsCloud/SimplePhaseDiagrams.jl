...
[SimplePhaseDiagrams](https://github.com/MineralsCloud/SimplePhaseDiagrams.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("SimplePhaseDiagrams")
   ```

3. Run

   ```julia-repl
   julia> using SimplePhaseDiagrams
   ```
...
version of SimplePhaseDiagrams, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/SimplePhaseDiagrams.jl"
```

in the second step above.

## Update SimplePhaseDiagrams
...
our [GitHub repository](https://github.com/MineralsCloud/SimplePhaseDiagrams.jl)
for new releases.
Once we release a new version, you can update SimplePhaseDiagrams by typing

```@repl
using Pkg
Pkg.update("SimplePhaseDiagrams")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall SimplePhaseDiagrams
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("SimplePhaseDiagrams")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall SimplePhaseDiagrams.
